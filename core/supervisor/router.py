...
# Worker replies whose text is already user-ready and must reach the user
# verbatim (after marker stripping) instead of being re-generated by the LLM.
# Workers run with the full Klaudia persona prompt, so their output is already
# in-voice; re-generating it only loses grounded content and adds an LLM hop:
#   [READ_DONE]  — carries data (tables, figures, totals). A re-gen summarizes
#                  rows away and can silently alter financial numbers.
#   [WRITE_DONE] — carries the written rows + before/after summary. The re-gen
#                  collapsed it into generic fluff ("Siap kak, noted").
#   [SHEET_DONE] — carries the structural change summary.
#   [CLARIFY]    — carries a question for the user. Re-genning it makes the LLM
#                  fabricate an answer around a question it cannot satisfy.
# _emit_final_reply remains the fallback only for a worker reply that somehow
# carries no marker at all.
_PASSTHROUGH_MARKERS = ("[READ_DONE]", "[WRITE_DONE]", "[SHEET_DONE]", "[CLARIFY]")
    ...
    async def supervisor_node(state: SupervisorState) -> Command[...]:
        ...
        if state_messages:
            ...
            if last_name in _WORKER_NAMES:
                ...
                logger.info(f"Supervisor: deterministic FINISH (rephrase from {last_name})")
